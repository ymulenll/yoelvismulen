...
export function Navbar() {
  ...
  return (
    <nav>
      <animated.ul
        className={
          'fixed bottom-0 z-10 flex w-screen items-stretch justify-around overflow-visible border-t bg-white shadow-2xl sm:h-full sm:w-auto sm:flex-col sm:items-start sm:justify-start sm:border-t-0 sm:border-r'
        }
        style={props}
      >
        {/* <li>
...
const Icon = ({
  ...
  return (
    <div className="relative">
      ...
      <div
        role="tooltip"
        className={`absolute z-20 w-max rounded bg-gray-500 px-4 py-1 font-mono text-white text-opacity-80 opacity-0
           transition duration-300 sm:top-[50%] sm:left-full sm:ml-1 sm:-translate-y-1/2 hover-hover:peer-hover:opacity-100`}
      >
        {title}
      </div>
    </div>
